...
def _adapter() -> FixtureOwnerContractAdapter:
    return FixtureOwnerContractAdapter.from_payloads(
        ...
        evidence_by_run={"run-owner-1": _load("evidence_bundle.json")},
        versions_by_run={"run-owner-1": _load("plan_versions.json")},
        diffs_by_run={
            (
                "run-owner-1",
                "run-owner-1:v1",
...
def _job(store: SQLiteJobStore, *, upstream_run_id: str | None = "run-owner-1"):
...
def test_owner_contract_fixtures_validate_at_adapter_boundary():
    ...
    assert adapter.get_evidence_bundle("run-owner-1").evidences[0].locator == {
        "page": 7,
        "section": "Results",
    }
    versions = adapter.list_plan_versions("run-owner-1")
