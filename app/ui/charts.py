...
def make_agent_timeline(agent_trace: list[dict]) -> go.Figure:
    ...
    """
    if not agent_trace:
        return _empty_fig("Agent Timeline")
    ...
    statuses = [ev.get("status", "") for ev in agent_trace]
    ...
    fig = go.Figure(
        go.Bar(
            ...
            text=statuses,
            textposition="auto",
            hovertemplate="%{y}<br>status=%{text}<br>%{x} ms<extra></extra>",
        )
    )
    fig.update_layout(
        title="Agent Pipeline Timeline（状态/耗时）",
...
def make_evidence_distribution(evidence_cards: list[dict]) -> go.Figure:
    ...
    if not evidence_cards:
        return _empty_fig("Evidence Source Distribution")
    ...
    fig = go.Figure(go.Bar(x=labels, y=values, marker=dict(color=colors), text=values, textposition="auto"))
    fig.update_layout(title="Evidence by Source Type", height=320, **_TRANSPARENT_LAYOUT)
    return fig
...
def make_relevance_histogram(evidence_cards: list[dict]) -> go.Figure:
    ...
    """
    if not evidence_cards:
        return _empty_fig("Relevance Score Distribution")
    scores = [float(c.get("relevance_score", 0) or 0) for c in evidence_cards]
    fig = go.Figure(go.Histogram(x=scores, nbinsx=10, marker=dict(color=theme.SCIENCE_CYAN)))
    fig.update_layout(title="Relevance Score Distribution", xaxis_title="relevance", yaxis_title="count", height=320, **_TRANSPARENT_LAYOUT)
    return fig
...
def make_readiness_radar(plan: dict) -> go.Figure:
    ...
    fig.update_layout(
        title="Rule-based readiness preview, not experimental validation.",
...
def make_domain_coverage_chart(questions: list[dict]) -> go.Figure:
    ...
    """
    if not questions:
        return _empty_fig("Domain Coverage")
    counts = Counter(q.get("domain", "Unknown") for q in questions)
    labels = list(counts.keys())
    values = [counts[k] for k in labels]
    colors = [theme.domain_color(k) for k in labels]
    fig = go.Figure(go.Bar(x=values, y=labels, orientation="h", marker=dict(color=colors), text=values, textposition="auto"))
    fig.update_layout(title="125 Questions · Domain Coverage", height=420, yaxis=dict(autorange="reversed"), **_TRANSPARENT_LAYOUT)
    return fig
